  ...
  uploadImage =()=> {
    ...
    r.onreadystatechange = function () {
      if(r.status === 200 && r.readyState === 4) {
        ...
        document.getElementById('selected-image').appendChild(i)
        document.getElementById('selected-image').appendChild(p)
      }
  ...
  render() {
    return (
      ...
      <form className='inputform'>
        <label for="inimage"><img src={uploadIcon} height={45} /></label>
          <input type="file" className="input-image" id='inimage' onChange={this.uploadImage.bind(this)} /> 
         </form>
      </div>
    );
  }
